...
<template>
  <div class="demo-all">
    <h1>Vue Koncept Demo</h1>
    
    <!-- Tabs för att visa olika exempel -->
    <div class="tabs">
      <button 
    ...
    <div class="content">
      ...
      <div v-if="activeTab === 'slots'">
        <SlotsExample>
          <p><strong>Detta är custom innehåll i basic slot!</strong></p>
        </SlotsExample>
        
        <hr>
        
        <!-- Användning med named slots -->
        <h3>Användning med named slots:</h3>
        <SlotsExample>
          <template #header>
            <h2>Min Custom Header</h2>
          </template>
          
          <p>Custom main content här!</p>
          
          <template #footer>
            <p>Custom footer innehåll</p>
          </template>

          <!-- Scoped slot användning -->
          <template #item="{ item }">
            <strong>{{ item.name }}</strong> kostar {{ item.price }} kr 
          </template>
        </SlotsExample>
      </div>
      ...
      <div v-if="activeTab === 'layout'">
        <LayoutExample>
          ...
          <h2>Huvudinnehåll</h2>
          <p>Detta är mitt custom innehåll i main-sektionen!</p>
